...
# ====================================
# 直線方向への旋回制御（比例制御）
STRAIGHT_TURN_GAIN = 0.02         # センサー距離差に対する操舵角の応答性（deg/mm）
FRONT_SENSOR_MARGIN = 50.0        # mm - 正面センサーがこの距離だけ最大なら完了とみなす
MAX_STEER_CHANGE_RATE = 10.0      # deg - 1ループあたりの最大ステアリング変化量（急激な変化を防ぐ）

# ステアリングスムージング（2センサーバージョンと同等）
# ====================================
STEERING_SMOOTHING_ENABLED = True  # ステアリングスムージングを有効化
STEERING_ALPHA = 0.3               # ローパスフィルタ係数（0.0-1.0、小さいほど滑らか）
