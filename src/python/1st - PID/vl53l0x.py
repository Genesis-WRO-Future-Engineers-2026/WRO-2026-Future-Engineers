...
class VL53L0X:
    ...
    def set_address(self, new_address):
        """Añade esta función al final de la clase VL53L0X de uceeatz"""
        # Escribe la nueva dirección en el registro I2C_SLAVE_DEVICE_ADDRESS (0x8A)
        self.i2c.writeto_mem(self.address, 0x8A, bytes([new_address & 0x7F]))
        # Actualiza la variable interna para que las lecturas apunten a la nueva dirección
        self.address = new_address
    
    def set_offset(self, offset_mm):
        """Ajusta el desfase de hardware en milímetros (acepta valores negativos)"""
        # Convierte el número entero a un valor de 16 bits firmado para el registro 0x28
        val = int(offset_mm * 4) # El registro usa cuartos de milímetro
        if val < 0:
            val = (1 << 16) + val
        high = (val >> 8) & 0xFF
        low = val & 0xFF
        self.i2c.writeto_mem(self.address, 0x28, bytes([high, low]))
