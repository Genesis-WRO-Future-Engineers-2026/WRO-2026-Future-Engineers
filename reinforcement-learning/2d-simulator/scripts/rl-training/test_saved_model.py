...
def test_saved_model(model_path: str, n_episodes: int = 3, render: bool = False, course_file: str = "courses/easy/simple_oval.json"):
    ...
    for episode in range(n_episodes):
        ...
        checkpoints_passed_list.append(info["next_checkpoint_index"])

        print(f"Episode {episode + 1}:")
        print(f"  Reward: {episode_reward:.2f}")
        print(f"  Length: {episode_length}")
        print(f"  Checkpoints: {info['next_checkpoint_index']}/{info['total_checkpoints']}")
    ...
    print(f"Average Checkpoints: {np.mean(checkpoints_passed_list):.1f}")
    print("=" * 60)
